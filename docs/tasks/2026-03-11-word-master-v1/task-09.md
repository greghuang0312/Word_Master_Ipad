...
- **范围**：输出 Supabase 配置、RLS、账号预建与排错文档
- **验收标准**：与 $plan 中 Task 9 定义一致（FAIL -> PASS 闭环）
- **依赖**：Task 03
...
- Step 2: 运行测试，确认 FAIL（保存日志到 logs/task-09-test-fail.log）
- Step 3: 编写最小实现代码
- Step 4: 运行测试，确认 PASS（保存日志到 logs/task-09-test-pass.log）
