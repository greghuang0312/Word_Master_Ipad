...
- **范围**：实现卡片显示、翻译揭示、会/不会判定与切词
- **验收标准**：与 $plan 中 Task 4 定义一致（FAIL -> PASS 闭环）
- **依赖**：Task 02, Task 03
...
- Step 2: 运行测试，确认 FAIL（保存日志到 logs/task-04-test-fail.log）
- Step 3: 编写最小实现代码
- Step 4: 运行测试，确认 PASS（保存日志到 logs/task-04-test-pass.log）
